...
import app.skydroid.BuildConfig;
...
public abstract class ShellSaiPackageInstaller extends BaseSaiPackageInstaller {
    ...
    private void install(String sessionId, SaiPiSessionParams params) {
        ...
        try (ApkSource apkSource = params.apkSource()) {
            ...
            if (!getShell().isAvailable()) {
                setSessionState(sessionId, new SaiPiSessionState.Builder(sessionId, SaiPiSessionStatus.INSTALLATION_FAILED).error(("installer_error_shell"+"|"+ getInstallerName()+"|"+ getShellUnavailableMessage()), null).build());
                unlockInstallation();
                return;
            }
            ...
            while (apkSource.nextApk()) {
                if (apkSource.getApkLength() == -1) {
                    setSessionState(sessionId, new SaiPiSessionState.Builder(sessionId, SaiPiSessionStatus.INSTALLATION_FAILED).appTempName(appTempName).error(("installer_error_unknown_apk_size"), null).build());
                    unlockInstallation();
                    return;
                }
            ...
            if (!installationResult.isSuccessful()) {
                ...
                String shortError = ("installer_error_shell"+"|"+ getInstallerName()+"|"+ getSessionInfo(apkSource) + "\n\n" + parseError(installationResult));
            ...
            setSessionState(sessionId, new SaiPiSessionState.Builder(sessionId, SaiPiSessionStatus.INSTALLATION_FAILED)
                    .appTempName(appTempName)
                    .error(("installer_error_shell"+"|"+ getInstallerName()+"|"+ getSessionInfo(params.apkSource()) + "\n\n" + e.getLocalizedMessage()), ("installer_error_shell" +"|"+getInstallerName()+"|"+ getSessionInfo(params.apkSource()) + "\n\n" + Utils.throwableToString(e)))
                    .build());
    ...
    private String getSessionInfo(ApkSource apkSource) {
        ...
        return String.format("%s: %s %s | %s | Android %s | Using %s ApkSource implementation | SAI %s", "installer_device", Build.BRAND, Build.MODEL, MiuiUtils.isMiui() ? "MIUI" : "Not MIUI", Build.VERSION.RELEASE, apkSource.getClass().getSimpleName(), saiVersion);
    }
    ...
    private int createSession() throws RuntimeException {
        ...
        if (customInstallCreateCommand != null) {
            ...
            Log.d(tag(), "Using custom install-create command: " + customInstallCreateCommand);
        } else {
