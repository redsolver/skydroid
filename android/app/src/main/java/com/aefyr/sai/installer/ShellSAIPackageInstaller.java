...
public abstract class ShellSAIPackageInstaller extends SAIPackageInstaller {
    ...
    protected void installApkFiles(ApkSource aApkSource) {
        try (ApkSource apkSource = aApkSource) {
            ...
            if (!getShell().isAvailable()) {
                dispatchCurrentSessionUpdate(InstallationStatus.INSTALLATION_FAILED, "installer_error_shell "+ getInstallerName()+ "\n\n"+ getShellUnavailableMessage());
                installationCompleted();
                return;
            }
            ...
            while (apkSource.nextApk()) {
                if (apkSource.getApkLength() == -1) {
                    dispatchCurrentSessionUpdate(InstallationStatus.INSTALLATION_FAILED, "installer_error_unknown_apk_size");
                    installationCompleted();
                    return;
                }
    ...
    private String getSessionInfo(ApkSource apkSource) {
        ...
        return String.format("%s: %s %s | %s | Android %s | Using %s ApkSource implementation | SAI %s", "installer_device "+ Build.BRAND +"\n"+ Build.MODEL+"\n" + ( MiuiUtils.isMiui() ? "MIUI" : "Not MIUI")+"\n"+ Build.VERSION.RELEASE+"\n" + apkSource.getClass().getSimpleName()+"\n" + saiVersion);
    }
